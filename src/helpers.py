...
def obtain_photo_category_label(image_path):
    '''
    ...
    '''
    model = resnet18(
        weights=ResNet18_Weights.IMAGENET1K_V1)  # create an instance of the ResNet18 model with the weights from the ImageNet dataset
    ...
    model.eval()  # set the model to evaluation mode, which means that the model will not change the weights during the evaluation, because
    ...
    with torch.no_grad():  # disable gradient calculation, because we are not training the model, we are just using it to make predictions
        outputs = model(
            img_tensor)  # output is the probability of each class, the model takes the image tensor as input and returns the output tensor
    ...
    probabilities = torch.nn.functional.softmax(outputs[0],
                                                dim=0)  # apply the softmax function to the output tensor, which converts the output
    ...
    class_label = ResNet18_Weights.IMAGENET1K_V1.meta["categories"][
        class_id]  # takes the name of the class with the highest probability
